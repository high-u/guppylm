...
WORD_DICT = [
    ...
    # ── 夢・目標グループ ──
    # noun
    {"word": "カイゾクオウ", "pos": "noun", "topics": ["goal"]},
    ...
    # adj
    {"word": "ツヨイ",       "pos": "adj", "topics": ["goal"]},
    {"word": "デッカイ",     "pos": "adj", "topics": ["goal"]},
    # reaction
    {"word": "ゼッタイ",     "pos": "reaction", "topics": ["goal"]},
    {"word": "キメタ",       "pos": "reaction", "topics": ["goal"]},
    {"word": "ガンバル",     "pos": "reaction", "topics": ["goal"]},
...
def build_sentence(pattern, nouns, verbs_tr, verbs_intr, verbs_ni, adjs):
    """パターンに従って1文を組み立てる。"""
    if pattern == "P1":
        return f"{random.choice(nouns)['word']} ハ {random.choice(adjs)['word']}"
    elif pattern == "P2":  # 他動詞: N ヲ Vtr
...
def generate_response(topic):
    ...
    verbs_ni = [w for w in pool if w["pos"] == "verb" and w.get("ni")]
    verbs_intr = [w for w in pool if w["pos"] == "verb" and not w.get("tr") and not w.get("ni")]
    ...
    # 利用可能なパターンを取得
    available = get_available_patterns(nouns, verbs_tr, verbs_intr, verbs_ni, adjs)
    ...
    sent2 = build_sentence(pattern2, nouns, verbs_tr, verbs_intr, verbs_ni, adjs)
    ...
    sent3 = build_sentence(pattern3, nouns, verbs_tr, verbs_intr, verbs_ni, adjs)
